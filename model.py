...
class BRC(object):
    ...
    def create_1x1(self,filters,num,tensor):
        x = layers.Conv2D(filters=filters,kernel_size=(1,1),strides=(1,1),name = 'conv'+str(num),padding='same',use_bias=False)(tensor)
        x = layers.BatchNormalization()(x)
        x = layers.LeakyReLU(alpha=0.1)(x)
        return x

    def create_3x3(self,filters,num,tensor):
        x = layers.Conv2D(filters=filters, kernel_size=(3, 3), strides=(1, 1), name='conv' + str(num),padding='same', use_bias=False)(tensor)
        x = layers.BatchNormalization()(x)
        x = layers.LeakyReLU(alpha=0.1)(x)
        return x

    def create_layer_down(self,filters,pool_size,num,tensor):
        x = layers.Conv2D(filters=filters,kernel_size=(3,3),strides=(1,1),name = 'conv'+str(num),padding='same',use_bias=False,kernel_initializer='random_uniform')(tensor)
        x = layers.BatchNormalization()(x)
        x = layers.LeakyReLU(alpha=0.1)(x)
    ...
    def create_layer_up(self,filters,up_size,num,tensor):
        ...
        x = layers.Conv2D(filters=filters, kernel_size=(3, 3), strides=(1, 1), name='conv' + str(num),padding='same', use_bias=False,kernel_initializer='random_uniform')(x)
        x = layers.BatchNormalization()(x)
        x = layers.LeakyReLU(alpha=0.1)(x)
        return x
    ...
    def create_model(self):
        ...
        Out = layers.Lambda(lambda x: np.subtract(np.multiply(x,5.0),tf.convert_to_tensor(np.array([[[0,2.5]]]).astype(np.float32))))(Out)
